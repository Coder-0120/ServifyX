...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .lp-nav-links { display: flex; gap: 2rem; list-style: none; align-items: center; }
  .lp-nav-link {
  ...
  .lp-mobile-link:hover { color: #a5b4fc; }
  ...
  return (
    <>
      ...
      <nav className="lp-nav">
        <div className="lp-logo" onClick={() => { navigate("/"); setMenuOpen(false); }}>ServifyX</div>
        <ul className="lp-nav-links">
          {NAV_LINKS.map(({ label, action }) => (
            <li key={label}><span className="lp-nav-link" onClick={() => { action(); setMenuOpen(false); }}>{label}</span></li>
          ))}
        </ul>
        <div className="lp-nav-auth" style={{ display: "flex", gap: ".6rem" }}>
          <button className="lp-nav-ghost" onClick={() => { navigate("/login"); setMenuOpen(false); }}>Login</button>
          <button className="lp-nav-solid" onClick={() => { navigate("/register"); setMenuOpen(false); }}>Sign Up</button>
        </div>
        <button type="button" className="lp-hamburger" onClick={() => setMenuOpen(open => !open)}>
      ...
      {menuOpen && (
        <div className="lp-mobile-menu">
          {NAV_LINKS.map(({ label, action }) => (
            <span key={label} className="lp-mobile-link" onClick={() => { action(); setMenuOpen(false); }}>{label}</span>
          ))}
